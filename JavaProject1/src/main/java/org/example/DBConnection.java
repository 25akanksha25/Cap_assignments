...
public class DBConnection {
        ...
        if(connection == null){
            ...
            synchronized (DBConnection.class){
                ...
                if(connection == null){
                    ...
                    try {
                        connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/persons", "root", "Chopra@10");
    ...
    public static void updatePerson(int id, Person newP)
    {
        try
        {
            PreparedStatement p = connection.prepareStatement("UPDATE person SET first_name = ?, last_name = ?, age = ? WHERE id = ?");
            p.setString(1,newP.getFirst_name());
            p.setString(2,newP.getLast_name());
            p.setInt(3,newP.getAge());
            p.setInt(4,id);


            int rowChange = p.executeUpdate();
            if(rowChange >= 1)
            {
